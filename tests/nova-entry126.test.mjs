...
test('entry quota draw happens once on a new AT, never on loss, resume, or reload',()=>{
 ...
 assert.equal(calls,1);assert.equal(fresh.entryQuota,'2000');assert.equal(fresh.remaining,'2000');assert.equal(fresh.sets,'1');assert.equal(fresh.setQuota,'150');
 const saved=a.normalize(JSON.parse(JSON.stringify(fresh)));assert.deepEqual(saved,fresh);
 const resumed=a.afterBonus(saved,{},1,rng);assert.equal(calls,1);assert.equal(resumed.remaining,'2000');assert.equal(resumed.sets,'2');assert.equal(resumed.entryQuota,'2000');
 const direct=a.enter({},rng);assert.equal(calls,2);assert.equal(direct.remaining,'2000');
});
...
test('all settings draw the same initial quota distribution with valid weights',()=>{
 ...
 for(const [roll,expected] of [[0,150],[.92999,150],[.93001,500],[.95001,1000],[.96001,2000],[.99999,2000]]){
...
test('extra sets keep their awarded quota, including old saved 300pt sets',()=>{
 ...
 const next=a.step(fresh,{},()=>.999,'BELL');assert.equal(next.flow.remaining,'150');assert.equal(next.flow.sets,'1');assert.equal(next.flow.entryQuota,'150');
});
...
test('entry quota never caps or changes later role rewards',()=>{
 ...
 assert.equal(raised.remaining,'3100');assert.equal(raised.entryQuota,'150');
});
...
test('comeback and ura challenge preserve the entry draw and earned balance',()=>{
 ...
 const revival=a.step(comeback,{setting:6},()=>.5,'STRONG_NOVA');assert.equal(revival.comebackEvent,'success');assert.equal(revival.flow.entryQuota,'2000');
 const win=a.step({...start,burstPending:true,burstUsed:true},{setting:6},()=>0);
 assert.equal(win.burstReward.type,'ura');assert.equal(win.burstReward.points,0);assert.equal(win.flow.entryQuota,'2000');assert.equal(win.flow.remaining,'2000');
});
