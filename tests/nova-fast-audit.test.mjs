...
test('fast-spin audit retains exact zone awards and remaining stock without mutating flow',()=>{
 ...
 assert.match(text,/20000pt/);assert.match(text,/残り22000pt/);assert.match(text,/特化ストック9個/);assert.ok(text.startsWith('[FAST]'));assert.equal(JSON.stringify(resolved),before);
});
