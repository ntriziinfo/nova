...
test('every strip completes one revolution in 700ms in either direction',()=>{for(const length of [4,21,30])for(const reverse of [false,true]){const t=setup(),strip=Array.from({length},(_,i)=>String(i));t.a.start(0,t.reel,strip,0,reverse,s=>s);t.tick(350);assert.equal(t.a.top(0),Math.round(length/2));t.tick(350);assert.equal(t.a.top(0),0);assert.equal(t.a.rotationMs,700);t.a.clearAll();}});

test('landing keeps 700ms rotation speed and stops at nearest matching occurrence',async()=>{
 for(const reverse of [false,true]){
  ...
  t.tick(700/6);await Promise.resolve();assert.equal(done,false);assert.equal(t.a.top(0),reverse?2:4);
  t.tick(700/6+.001);await p;assert.equal(done,true);assert.equal(t.a.top(0),3);
 }
});
