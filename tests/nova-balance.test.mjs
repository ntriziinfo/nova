...
test('profiles show measured complete-stop RTP for the adopted 30000G burst design',()=>{
 const report=JSON.parse(fs.readFileSync('docs/burst113-summary.json','utf8'));
 for(let i=0;i<6;i++){const p=b.profile(i+1);assert.equal(p.target,b.targets[i]);assert.equal(p.verifiedModel,"burst113-30000g-complete-stop");assert.ok(Math.abs(p.target-report.settings[i].report.stoppedRtp.value)<1e-6);assert.ok(p.scale>0);}
});
