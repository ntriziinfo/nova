...
test('new mode profiles retain the six targets with positive fitted entry scales',()=>{
 for(let i=0;i<6;i++){const p=b.profile(i+1);assert.equal(p.target,b.targets[i]);assert.equal(p.verifiedModel,"chain-zones-v6");assert.ok(p.scale>0);if(i)assert.ok(p.directDenom<b.profile(i).directDenom);}
});
