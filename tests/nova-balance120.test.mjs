...
test('ura reward guarantees one of the three ura zones and no automatic level promotion',()=>{
 ...
 for(const [roll,zone] of [[0,'ura_giru'],[.5,'ura_sora'],[.99,'ura_ouma']]){
  ...
  const won=a.step(base,{setting:6},()=>values.shift());assert.equal(won.burstEvent,'success');assert.equal(won.burstReward.zone,zone);assert.equal(won.flow.remaining,'123');assert.equal(won.flow.atLevel,undefined);assert.equal(won.flow.entryStage,'confirmed');
...
test('ura challenges retain three-game 50% success and a used flag after failure',()=>{
 loadCandidate({setting:6});const a=NovaArt;
 for(const burstType of ['ura']){
