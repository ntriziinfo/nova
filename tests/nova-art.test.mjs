...
test('Giru changes continuation at 80 and 320G with mean 80G across settings',()=>{
 for(let setting=1;setting<=6;setting++){
  ...
  for(const [g,p]of [[5,row[0]],[79,row[0]],[80,row[1]],[319,row[1]],[320,row[2]],[640,row[2]]]){
   ...
   let state=a.startZone(a.enter(),'giru',{setting});state.award=String(g);state.giruBase=5;
 ...
 let s=a.startZone(a.enter(),'giru',{setting:6});s.award='40';s.giruBase=5;let t=a.step(s,{},()=>0);
 assert.equal(t.flow.award,'80');assert.match(t.message,/逆回転成功/);
...
test('Sora adds ART sets directly without BIG stock or a bonus',()=>{
 let s=a.startZone(a.enter(),'sora');for(let i=0;i<10;i++){const t=a.step(s,{soraHit:1,soraReset:0},()=>0);assert.equal(t.result,'BIG');assert.equal(t.internalBonus,null);s=t.flow;}assert.equal(s.stock,'0');assert.equal(s.sets,'10');
...
test('Ouma super adds 200G without normal freeze and Urapi yields 40G mean target',()=>{
 let s=a.startZone(a.enter(),'ouma',{oumaGames:1});s.awardTier=4;let seq=[.99,0,.99];const t=a.step(s,{oumaFreeze:0},()=>seq.shift());assert.equal(t.result,'SUPER_NOVA');assert.equal(t.internalBonus,null);assert.equal(a.prepareBet(t.flow,{oumaFreeze:0},()=>.99).remaining,'250');
