...
test('Giru changes continuation at 80 and 320G with mean 80G across settings',()=>{
 for(let setting=1;setting<=6;setting++){
  ...
  for(const [g,p]of [[5,row[0]],[79,row[0]],[80,row[1]],[319,row[1]],[320,row[2]],[640,row[2]]]){
   ...
   let state=a.startZone(a.enter(),'giru',{setting});state.award=String(50+g);state.giruBase=5;
   assert.equal(a.step(state,{},()=>p).flow.zone,'');
   assert.equal(a.step(state,{},()=>p-1e-9).flow.award,String(50+g*2));
  }
  assert.ok(row[0]>row[1]&&row[1]>row[2]&&row[2]<.5);
 }
 let s=a.startZone(a.enter(),'giru',{setting:6});s.award='90';s.giruBase=5;let t=a.step(s,{},()=>0);
 assert.equal(t.flow.award,'130');assert.match(t.message,/逆回転成功/);
 s=a.normalize(JSON.parse(JSON.stringify(t.flow)));assert.equal(s.giruSetting,6);
 t=a.step(s,{},()=>.56);assert.equal(t.flow.zone,'');assert.equal(t.flow.remaining,'180');
});
...
test('Giru has exact unlimited doubling, survives JSON, and adds final value once',()=>{
 ...
 assert.equal(s.award,(50n+5n*2n**1100n).toString());assert.equal(s.remaining,'50');assert.equal(s.zero,true);
 s=a.step(s,{},()=>.5).flow;assert.equal(s.remaining,(100n+5n*2n**1100n).toString());assert.equal(s.zero,false);assert.equal(s.zone,'');
});
...
test('Ouma super adds 200G without normal freeze and Urapi yields 40G mean target',()=>{
 let s=a.startZone(a.enter(),'ouma',{oumaGames:1});s.awardTier=4;let seq=[.99,0,.99];const t=a.step(s,{oumaFreeze:0},()=>seq.shift());assert.equal(t.result,'SUPER_NOVA');assert.equal(t.internalBonus,null);assert.equal(a.prepareBet(t.flow,{oumaFreeze:0},()=>.99).remaining,'300');
