...
test('one initial hit accumulates bonus and ART gross payouts without a 999 cap',()=>{
 ...
 assert.equal(step(start.state,true,250,360).count,150);
 assert.equal(step(start.state,true,2500,700).count,2400);
 const resumed=JSON.parse(JSON.stringify(start.state));assert.equal(step(resumed,true,2500,700).count,2400);
});
test('normal return resets the chain and a fresh hit starts at zero',()=>{
 ...
 assert.equal(step(fresh.state,true,2616,906).count,16);
});
test('BET costs do not reduce the displayed winnings, and reset totals discard stale baselines',()=>{
 const base=step(null,true,100,300).state;
 assert.equal(step(base,true,100,310).count,0);assert.equal(step(base,true,116,313).count,16);
