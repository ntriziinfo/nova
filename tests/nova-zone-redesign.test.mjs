...
test('nine zones and retired aliases preserve queued awards without new promotion',()=>{
 ...
 for(const base of ['sosuke','toto','urapi']){
  assert.equal(a.startZone(({...a.enter({},()=>.5),remaining:'150',entryQuota:'150'}),'ura_'+base,{},()=>0).ura,false);
  assert.equal(a.startZone(({...a.enter({},()=>.5),remaining:'150',entryQuota:'150'}),base,{allowUra:true},()=>0).ura,false);
...
test('first game reveals secured rung, four successes grant highest only in five paid games',()=>{
 for(const id of ['sosuke','giru','ura_giru']){
  let s=a.startZone(({...a.enter({},()=>.5),remaining:'150',entryQuota:'150'}),id,{},()=>0);const steps=Array.from(s.ladder);
...
test('50 to 100 then MISS settles exactly 100 and sixth table can reach 3000',()=>{
 let s=a.startZone(({...a.enter({},()=>.5),remaining:'150',entryQuota:'150'}),'giru',{},()=>0);s=a.step(s,{},()=>0).flow;s=a.step(s,{},()=>0,'REPLAY').flow;
 s=a.step(s,{},()=>0,'MISS').flow;assert.equal(s.zone,'');assert.equal(s.remaining,'250');
 s=a.startZone(({...a.enter({},()=>.5),remaining:'150',entryQuota:'150'}),'ura_giru',{},()=>.9);for(let i=0;i<5;i++)s=a.step(s,{},()=>0,'BELL').flow;
...
test('analytic expectations agree with independent simulation and strength ordering',()=>{
 ...
 for(const id of a.zoneIds){let total=0;
  for(let i=0;i<15000;i++){let s=a.startZone(({...a.enter({},()=>.5),remaining:'150',entryQuota:'150'}),id,{},rng),g=0;while(s.zone&&g++<10000){s=a.prepareBet(s,{},rng);if(s.zone)s=a.step(s,{},rng).flow;}assert.equal(s.zone,'');total+=Number(s.remaining)-150;}
