...
test('100000 actual BIGs per tier reproduce AT expectations; shown cue colors retain their confidence',()=>{
 for(const tier of ['normal','upper']){
  ...
  for(let i=0;i<100000;i++){
   ...
   while(paid<50){const result=a.drawBonus(rng,1,tier);paid+=a.bonusPayout({paid},result);hit ||= result==='NEBULA';const aim=a.bonusAim(result,rng);if(aim?.guide){assert.equal(aim.symbol,'nebula');colors[aim.color][0]++;colors[aim.color][1]+=result==='NEBULA';}}
   assert.equal(paid,50);won+=hit;
  }
...
test('live bonus resolver uses the same payout cap, NEBULA guide and AT award',()=>{
 ...
 assert.equal(ctx.resolveATypeBonusOutcome('BELL').reward,5);
 ctx.session.paid=50;assert.equal(ctx.resolveATypeBonusOutcome('NEBULA').artSetWon,0);assert.equal(ctx.isATypeBonusComplete(),true);
});
