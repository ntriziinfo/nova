...
test('net restraint has smooth setting-specific thresholds and a nonzero floor',()=>{
 for(let setting=1;setting<=5;setting++){const t=[2000,2000,4000,4000,5000][setting-1];assert.equal(a.netRewardFactor(setting,-2000),1);assert.equal(a.netRewardFactor(setting,0),1);assert.ok(Math.abs(a.netRewardFactor(setting,t*.5)-.525)<1e-12);assert.ok(Math.abs(a.netRewardFactor(setting,t)-.05)<1e-12);assert.ok(a.netRewardFactor(setting,t*100)>0);}
...
test('setting6 and negative net preserve both outcomes and random consumption',()=>{
 for(const [setting,netPt] of [[6,20000],[1,-2000]]){let x=0,y=0;const s=a.enter();assert.deepEqual(a.step(s,{setting,netPt},()=>{x++;return .1;},'STRONG_NOVA'),a.step(s,{setting,netPt:0},()=>{y++;return .1;},'STRONG_NOVA'));assert.equal(x,y);}
});
