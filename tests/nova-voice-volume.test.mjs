...
function setup(){
 ...
 for(const name of ['soundOutputVolume','aimWinOutputVolume','applyCharacterVoiceOutputVolumes','playOneShotSound','playAimSevenWinSound','fadeAimWinSoundsOnBet'])vm.runInContext(fn(name),c);
 return {c,Audio,tick(ms){now=ms;frames.forEach(f=>f());}};
}
