...
globalThis.NovaReelMotion=(()=>{
 ...
 function start(i,reel,strip,top,reverse,html){
  ...
  const s={win,layer,strip,h:cellHeight,pos:strip.length+mod(top,strip.length),direction:reverse?1:-1,last:performance.now(),raf:0};active.set(i,s);
  function frame(now){if(active.get(i)!==s)return;
   ...
   else{s.pos=strip.length+mod(s.pos-strip.length+s.direction*(now-s.last)/110,strip.length);}
 ...
 function stop(i,column){const s=active.get(i);if(!s)return Promise.resolve(false);const target=s.strip.findIndex((_,n)=>column.every((v,j)=>s.strip[(n+j)%s.strip.length]===v));
  ...
  return new Promise(resolve=>{s.resolve=resolve;s.landing={from:s.pos,to:s.pos+s.direction*distance,time:performance.now(),duration:Math.max(140,distance*110*2)};});
 }
 return {start,stop,clear,has:i=>active.has(i),top:i=>{const s=active.get(i);return s?mod(Math.round(s.pos),s.strip.length):null;},clearAll:()=>[...active.keys()].forEach(clear)};
})();
