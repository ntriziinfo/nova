...
await page.addInitScript(()=>{
 ...
 HTMLMediaElement.prototype.play=function(){
  this.muted=true;
  if(this.src.includes('/nova/bell-navi.wav')){
   const root=document.querySelector('#novaBellNavi'),event={visible:!root.hidden,time:performance.now(),loop:this.loop,volume:this.volume,played:false};
   bellSounds.push(event);const promise=play.call(this);promise.then(()=>{event.played=true;event.duration=this.duration;},e=>event.error=e.message);return promise;
  }
...
async function hooks(){await page.evaluate(()=>{
 window.qa={navShows:[],spins:[],forced:'BELL',next:0,autoStopAt:0};
 const begin=NovaBellNavi.begin,clear=NovaBellNavi.clear,step=NovaArt.step;
 NovaArt.step=(s,o,r,f)=>step(s,o,r,qa.forced||f);
 NovaBellNavi.begin=s=>{const result=begin(s,()=>(qa.next++%6+.5)/6);qa.spins.push(s);if(result)qa.navShows.push({time:performance.now(),order:result.slice()});return result;};
...
try{
 ...
 for(let p=0;p<6;p++){
  ...
  if(p===0){const n=await box(),r=await page.locator('.reels').boundingBox();assert(Math.abs(n.x-r.x)<1);assert(n.y+n.height<=r.y);assert(Math.abs(n.width-r.width)<1);await page.screenshot({path:'research/navi135/bell.png'});}
 ...
 const events=await page.evaluate(()=>({sounds:bellSounds,shows:qa.navShows}));
 assert.equal(events.sounds.length,13);assert.equal(events.shows.length,13);
 events.sounds.forEach((s,i)=>{assert(s.visible&&s.played&&!s.loop);assert(s.volume>0);assert(s.duration>0);assert(Math.abs(s.time-events.shows[i].time)<50);});checks.push({soundEvents:events});
 ...
 await page.screenshot({path:'research/navi135/adjust.png'});await page.locator('#layoutEditSaveDefaultBtn').click();await page.locator('#layoutEditCloseBtn').click();assert(await page.locator('#novaBellNavi').isHidden());
 assert.equal(await page.evaluate(()=>bellSounds.length),13);await page.reload({waitUntil:'domcontentloaded'});await ready();await page.locator('#bellNaviAdjustBtn').click();
 ...
 fs.writeFileSync('research/navi135/qa.json',JSON.stringify({silent:true,isolated:true,checks,pageErrors:errors,missingAssets:missing},null,2));console.log(JSON.stringify({checks:checks.length,pageErrors:errors,missing}));
}catch(e){await page.screenshot({path:'research/navi135/error.png'});console.log(JSON.stringify({error:String(e),errors,missing,qa:await page.evaluate(()=>window.qa)}));throw e;}finally{await browser.close();}
