...
function run(c){
 ...
 report.episodes={completed:done.length,censored:episodes.length-done.length,mean:done.reduce((a,e)=>a+e.paid,0)/done.length,reach2000:episodes.filter(e=>e.paid>=2000).length/episodes.length,mid:done.filter(e=>e.paid>=2000&&e.paid<4000).length/done.length};
 ...
 fs.writeFileSync(`research/common125/${c.tag}-${c.setting}.json`,JSON.stringify({report,rows})+'\n');
