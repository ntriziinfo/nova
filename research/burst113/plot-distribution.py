...
data=json.loads(Path(f'docs/burst113-{tag}-distribution.json').read_text(encoding='utf-8'))
...
for mode,label in [('stopped','コンプリート停止あり'),('unlimited','コンプリート停止なし')]:
    ...
    fig.suptitle(f'＋2,000pt・AT Lv.5案｜30,000Gの出玉分布',fontsize=23,fontweight='bold',x=.07,ha='left',y=.978)
    ...
    output=Path(f'docs/burst113-{tag}-{mode}-distribution.png')
