...
try{
 ...
 await page.evaluate(()=>qa.entryRoll=.99);await force('ART');const large=await spin();assert.equal(large.flow.remaining,'2000');assert.equal(large.flow.entryQuota,'2000');
 ...
 assert.match(await page.locator('#novaEntryQuotaTable').textContent(),/2000pt/);
 await force('URA_CHALLENGE');const entry=await spin();assert.equal(entry.flow.remaining,'150');assert.equal(entry.flow.burstType,'ura');assert.equal(entry.flow.atLevel,undefined);
 const misses=[];for(let i=0;i<3;i++){const r=await spin();misses.push(r);assert.equal(r.step.burstEvent,i===2?'failure':'continue');}
 assert.equal(misses.at(-1).flow.remaining,'150');
 await force('URA_CHALLENGE');await spin();await page.evaluate(()=>qa.mode='ura');const won=await spin();assert.equal(won.step.burstReward.zone,'ura_sora');assert.equal(won.flow.remaining,'150');assert.equal(won.flow.atLevel,undefined);
 await page.screenshot({path:'research/start126/qa-ura.png'});
 ...
 fs.writeFileSync('research/start126/qa-restored.json',JSON.stringify(restored.map(x=>({flow:x.normalState.flow,settings:{initial:x.settings.novaArt.initial,version:x.settings.entryQuotaVersion}})),null,2));
 assert(restored.every(x=>x.normalState.flow.stock==='2'&&x.normalState.flow.sets==='3'&&x.normalState.flow.setQuota==='300'));assert(restored.every(x=>x.settings.novaArt.initial===150&&x.settings.entryQuotaVersion===126));
 assert(restored.every(x=>x.runtimeState.forceResult===''&&x.runtimeState.pendingForceResult===''));
 assert.deepEqual(errors,[]);
 const report={initial2000:true,entryQuotaTable:true,oldSetQuotaPreserved:true,silent:true,isolated:true,manualFailure:true,manualSuccess:true,autoSuccess:true,fastAuditRecords:records.length,commonAtRecords:at.length,oldLevelAndPointChallengeRetired:true,earnedQuotaAndStocksPreserved:true,oldForceFlagsCleared:true,pageErrors:errors};
 fs.writeFileSync('research/start126/qa.json',JSON.stringify(report,null,2)+'\n');console.log(JSON.stringify(report));
}catch(e){await page.screenshot({path:'research/start126/qa-error.png'});console.log(JSON.stringify({error:String(e),pageErrors:errors}));throw e;}finally{await browser.close();}
