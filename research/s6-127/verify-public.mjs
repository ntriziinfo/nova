...
const sources=await Promise.all(files.map(async f=>{const r=await fetch(base+'/'+f+'?s6-127='+Date.now(),{headers:{'Cache-Control':'no-cache'}});assert.equal(r.status,200);const text=await r.text();assert.equal(hash(text),hash(fs.readFileSync(f,'utf8')),f);return {file:f,sha256:hash(text),matches:true};}));
...
try{
 ...
 const state=await page.evaluate(()=>({pointFlag:!!document.querySelector('#forceResult option[value="BURST"]'),uraFlag:!!document.querySelector('#forceResult option[value="URA_CHALLENGE"]'),level:NovaArt.enter({},()=>.5).atLevel,initial:NovaArt.enter({},()=>.5).remaining,common:NovaArt.commonAtRules,rtp:NovaBalance.targets,model:NovaBalance.profile(6).verifiedModel,normalBoost:NovaArt.burstRules.normalBoost,cz:NovaFlow.forSetting({},6)}));
 assert.equal(state.pointFlag,false);assert(state.uraFlag);assert.equal(state.level,undefined);assert.equal(state.initial,'150');assert.equal(state.model,'s6-127-30000g-complete-stop');assert.deepEqual(errors,[]);
 const measured=JSON.parse(fs.readFileSync('docs/s6-127-summary.json')).setting6;
 assert(Math.abs(state.rtp[5]-measured.rtp.value)<1e-6);
 assert.deepEqual(state.normalBoost,[.376,.39,.397,.448,.483,.584]);
 assert(Math.abs(state.cz.czChance-.6336)<1e-10);assert(Math.abs(state.cz.strongChance-.8168)<1e-10);
 const result={url:base+'/jag.html?debug=1',sources,state,pageErrors:errors,silent:true,isolated:true};fs.writeFileSync('research/s6-127/public-check.json',JSON.stringify(result,null,2));console.log(JSON.stringify(result));
