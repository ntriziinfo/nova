...
try{
 ...
 assert.equal(await page.locator('#novaMode').count(),0);
 assert.equal(await page.locator('#novaNormalGames').getAttribute('max'),'800');
 assert.equal(await page.evaluate(()=>NovaNormal.ceiling(NovaNormal.reset(()=>.5,6))),800);
 assert(!await page.locator('#novaArtConfig').textContent().then(s=>s.includes('Lv.5')));
 await page.evaluate(()=>{
  window.qa={flow:null,spinning:false,mode:'fail',steps:[],entryRoll:.5};const enter=NovaArt.enter;NovaArt.enter=(c)=>enter(c,()=>qa.entryRoll);const sync=NovaComeback.sync,step=NovaArt.step;
 ...
 await force('STRONG_NOVA');const rareCz=await spin();assert.equal(rareCz.flow.phase,'strong_cz');
 await page.locator('#debugToggleBtn').click();await page.locator('#morningResetBtn').click();
 await page.locator('#novaNormalConfig').evaluate(e=>e.open=true);await page.locator('#novaNormalGames').fill('49');await page.locator('#novaApplyInternal').click();await page.locator('#debugCloseBtn').click();
 await force('BELL');const noZone=await spin();assert.equal(noZone.flow.phase,'normal');
 await page.evaluate(()=>qa.entryRoll=.99);await force('ART');const large=await spin();assert.equal(large.flow.remaining,'1000');assert.equal(large.flow.entryQuota,'1000');
 await page.locator('#debugToggleBtn').click();await page.locator('#morningResetBtn').click();await page.locator('#debugCloseBtn').click();await page.evaluate(()=>qa.entryRoll=.5);
 assert.match(await page.locator('#novaEntryQuotaTable').textContent(),/1000pt/);
 await force('URA_CHALLENGE');const entry=await spin();assert.equal(entry.flow.remaining,'750');assert.equal(entry.flow.burstType,'ura');assert.equal(entry.flow.atLevel,undefined);
 const misses=[];for(let i=0;i<3;i++){const r=await spin();misses.push(r);assert.equal(r.step.burstEvent,i===2?'failure':'continue');}
 assert.equal(misses.at(-1).flow.remaining,'750');
 await force('URA_CHALLENGE');await spin();await page.evaluate(()=>qa.mode='ura');const won=await spin();assert.equal(won.step.burstReward.zone,'ura_sora');assert.equal(won.flow.remaining,'750');assert.equal(won.flow.atLevel,undefined);
 await page.screenshot({path:'research/role128/qa-ura.png'});
 ...
 const saved=await page.evaluate(()=>{
  const entries={};for(const key of Object.keys(localStorage).filter(k=>k.startsWith('nova_'))){let d;try{d=JSON.parse(localStorage.getItem(key));}catch{continue;}if(!d?.settings||!d?.normalState)continue;
   d.settings.novaArt.atLevelWeights=[0,0,0,0,100];delete d.settings.commonAtVersion;delete d.settings.entryQuotaVersion;d.settings.novaArt.initial=300;
   d.normalState.internal={mode:'天国',games:321,impurity:67,level:'high',highLeft:8,prelude:{kind:'fake',left:4}};
   ...
   delete d.normalState.flow.entryQuota;delete d.normalState.flow.setQuota;
 ...
 fs.writeFileSync('research/role128/qa-restored.json',JSON.stringify(restored.map(x=>({flow:x.normalState.flow,settings:{initial:x.settings.novaArt.initial,version:x.settings.entryQuotaVersion}})),null,2));
 assert(restored.every(x=>x.normalState.flow.stock==='2'&&x.normalState.flow.sets==='3'&&x.normalState.flow.setQuota==='300'));assert(restored.every(x=>x.settings.novaArt.initial===150&&x.settings.entryQuotaVersion===128));
 assert(restored.every(x=>x.runtimeState.forceResult===''&&x.runtimeState.pendingForceResult===''));
 assert(restored.every(x=>x.normalState.internal.mode==='通常'&&x.normalState.internal.games===321&&x.normalState.internal.impurity===67&&x.normalState.internal.prelude===null));
 assert.deepEqual(errors,[]);
 const report={initial1000:true,rareRoleCz:true,no50GZone:true,common800GCeiling:true,legacyNormalMigration:true,entryQuotaTable:true,oldSetQuotaPreserved:true,silent:true,isolated:true,manualFailure:true,manualSuccess:true,autoSuccess:true,fastAuditRecords:records.length,commonAtRecords:at.length,oldLevelAndPointChallengeRetired:true,earnedQuotaAndStocksPreserved:true,oldForceFlagsCleared:true,pageErrors:errors};
 fs.writeFileSync('research/role128/qa.json',JSON.stringify(report,null,2)+'\n');console.log(JSON.stringify(report));
}catch(e){await page.screenshot({path:'research/role128/qa-error.png'});console.log(JSON.stringify({error:String(e),pageErrors:errors}));throw e;}finally{await browser.close();}
