...
globalThis.NovaArt=(()=>{
 ...
 function normalize(v){return {queuedZones:Array.isArray(v?.queuedZones)?v.queuedZones.filter(z=>zoneIds.includes(z)):[],phase:'art',remaining:integer(v?.remaining).toString(),zone:names[baseZone(v?.zone)]?baseZone(v.zone):'',ura:!!v?.ura||String(v?.zone||'').startsWith('ura_'),zoneLeft:Math.max(0,Math.floor(Number(v?.zoneLeft)||0)),award:integer(v?.award).toString(),stock:integer(v?.stock).toString(),zoneSets:integer(v?.zoneSets).toString(),sets:integer(v?.sets).toString(),oumaPending:!!v?.oumaPending,zero:!!v?.zero,color:v?.color||'white',awardTier:[1,4,10,70].includes(v?.awardTier)?v.awardTier:4,giruVersion:v?.giruVersion===3?3:0,giruBase:Math.min(5,Math.max(0,Math.floor(Number(v?.giruBase)||0))),giruContinues:Math.max(0,Math.floor(Number(v?.giruContinues)||0)),giruSetting:validSetting(v?.giruSetting)};}
 function enter(c){return normalize({remaining:config(c).initial});}
 function startZone(s,z,c,rng=Math.random){s=normalize(s);const setting=validSetting(c?.setting),allowUra=!!c?.allowUra,explicitUra=String(z).startsWith('ura_');z=baseZone(z);c=config(c);if(!names[z])return s;const ura=explicitUra||(allowUra&&rng()<c.uraChance);const tier=['sosuke','toto','urapi','ouma'].includes(z)?pickAwardTier(rng):4;return {...s,giruVersion:3,giruBase:0,giruContinues:0,ura,oumaPending:false,zoneSets:'0',awardTier:tier,giruSetting:setting,zone:z,zoneLeft:{sosuke:3,toto:5,urapi:c.urapiGames,giru:2,sora:10,ouma:c.oumaGames}[z],award:'0',zero:false,color:'white'};}
 ...
 function step(value,options={},rng=Math.random,forced=''){
  ...
  if(s.zone){
   ...
   if(z==='giru'){result='MISS';if(s.giruVersion!==3){finish();}else if(s.giruBase===0){s.giruBase=1+Math.min(4,Math.floor(rng()*5));s.award=String(s.giruBase);s.zoneLeft=1;s.zero=false;message=zoneName(s)+' 基準'+s.giruBase+'G / 次Gから継続抽選';}else if(rng()<giruChance(s.award,s.giruSetting,s.ura)){s.award=(integer(s.award)*2n).toString();s.giruContinues++;s.zero=true;reverse=true;message=zoneName(s)+' 逆回転成功！ '+s.award+'G / '+s.giruContinues+'回継続';}else finish();}
   if(z==='sosuke'||z==='toto'){
