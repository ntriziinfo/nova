...
globalThis.NovaArt=(()=>{
 ...
 const defaults={initial:50,rare:.03,big:.12,zone:.4,czArt:.25,urapiGames:5,urapiHit:.4,oumaGames:5,soraHit:.25,oumaHit:.6,direct1:1400,direct2:1320,direct3:1240,direct4:1160,direct5:1080,direct6:1000};
 ...
 const settingBias=[-.765,-.60,-.425,-.117,.29,.77];
 ...
 const giruRates=Object.freeze([[.49,.29,.14],[.491,.291,.141],[.492,.292,.142],[.493,.293,.143],[.494,.294,.144],[.495,.295,.145]].map(Object.freeze));
 ...
 function normalize(v){return {queuedZones:Array.isArray(v?.queuedZones)?v.queuedZones.filter(z=>names[z]):[],phase:'art',remaining:integer(v?.remaining).toString(),zone:names[v?.zone]?v.zone:'',zoneLeft:Math.max(0,Math.floor(Number(v?.zoneLeft)||0)),award:integer(v?.award).toString(),stock:integer(v?.stock).toString(),sets:integer(v?.sets).toString(),zero:!!v?.zero,color:v?.color||'white',awardTier:[1,4,10,70].includes(v?.awardTier)?v.awardTier:4,giruSetting:validSetting(v?.giruSetting)};}
 function enter(c){return normalize({remaining:config(c).initial});}
 function startZone(s,z,c,rng=Math.random){s=normalize(s);const setting=validSetting(c?.setting);c=config(c);const tier=['sosuke','toto','urapi','ouma'].includes(z)?pickAwardTier(rng):4;return {...s,awardTier:tier,giruSetting:setting,zone:z,zoneLeft:{sosuke:3,toto:5,urapi:c.urapiGames,giru:1,sora:10,ouma:c.oumaGames}[z],award:z==='giru'?'5':'0',zero:false,color:'white'};}
 ...
 function swingAward(base,s,rng=Math.random){const scaled=base*s.awardTier/4,whole=Math.floor(scaled),fraction=scaled-whole;return whole+(fraction>0&&rng()<fraction?1:0);}
 ...
 function step(value,options={},rng=Math.random,forced=''){
  ...
  if(forced.startsWith('ZONE_')){s=startZone(s,forced.slice(5),{...c,setting:options.setting},rng);return {result,flow:s,message:names[s.zone]+'ゾーン突入'};}
  if(!s.zone&&s.queuedZones.length){queuedEntered=true;const z=s.queuedZones.shift();s=startZone(s,z,{...c,setting:options.setting},rng);}
  ...
  if(s.zone){
   ...
   if(z==='urapi'){if(forced==='NEBULA'||rng()<c.urapiHit){result='NEBULA';const n=swingAward(20,s,rng);s.award=(integer(s.award)+BigInt(n)).toString();message=`うらぴ nebula揃い ＋${n}G`;}if(!s.zoneLeft)finish();}
 ...
 return {pickAwardTier,swingAward,names,defaults,direct,zoneWeights,pickZone,bonusSpecial,settingBias,biasFor,bonusSpecialFor,advanceBonus,drawPaidRole,drawBonus,giruRates,giruChance,config,normalize,enter,startZone,afterBonus,step,label};
})();
