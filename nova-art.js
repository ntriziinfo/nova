...
globalThis.NovaArt=(()=>{
 ...
 function giruChance(award,setting=1,ura=false){const g=integer(award),rates=ura?giruRates.ura:giruRates.normal;return rates[g<(ura?160n:80n)?0:g<(ura?640n:320n)?1:2];}
 ...
 function swingAward(base,s,rng=Math.random,extra=1){const scaled=base*s.awardTier/4*(s.ura?({sosuke:5,toto:10/3,urapi:3.75,ouma:3}[s.zone]||1):1)*extra,whole=Math.floor(scaled),fraction=scaled-whole;return whole+(fraction>0&&rng()<fraction?1:0);}
 ...
 function settleZone(value){const s=normalize(value);if(!s.zone)return s;if(s.zone!=='sora')s.remaining=(integer(s.remaining)+integer(s.award)).toString();s.zone='';s.zoneLeft=0;s.zero=false;s.oumaPending=false;return s;}
 ...
 function step(value,options={},rng=Math.random,forced=''){
  ...
  if(s.zone){
   ...
   if(z==='giru'){result='MISS';if(s.giruVersion!==3){finish();}else if(s.giruBase===0){s.giruBase=1+Math.min(4,Math.floor(rng()*5));s.award=(integer(s.initialAward)+BigInt(s.giruBase)).toString();s.zoneLeft=1;s.zero=false;message=zoneName(s)+' 基準'+s.giruBase+'G / 次Gから継続抽選';}else if(rng()<giruChance(integer(s.award)-integer(s.initialAward),s.giruSetting,s.ura)){s.award=(integer(s.initialAward)+(integer(s.award)-integer(s.initialAward))*2n).toString();s.giruContinues++;s.zero=true;reverse=true;message=zoneName(s)+' 逆回転成功！ '+s.award+'G / '+s.giruContinues+'回継続';}else finish();}
   if(z==='sosuke'||z==='toto'){
   ...
   if(z==='ouma'){if(!['WEAK_NOVA','STRONG_NOVA','SUPER_NOVA'].includes(forced)&&(oumaZero||rng()<c.oumaHit)){const r=rng();result=r<.7?'WEAK_NOVA':r<.95?'STRONG_NOVA':'SUPER_NOVA';}const n=swingAward({WEAK_NOVA:20,STRONG_NOVA:50,SUPER_NOVA:200}[result]||0,s,rng,1-oumaFreezeRate(s,c));s.award=(integer(s.award)+BigInt(n)).toString();message=n?`${oumaZero?'逢魔フリーズ！ 0G連 / ':''}${zoneName(s)} ${result} ＋${n}G`:'ノヴァを狙え';s.oumaPending=['WEAK_NOVA','STRONG_NOVA','SUPER_NOVA'].includes(result);s.zero=false;if(!s.zoneLeft&&!s.oumaPending)finish();}
  }else{
