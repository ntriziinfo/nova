...
globalThis.NovaNormal=(()=>{
 ...
 function normalize(v){return {morningCeiling:v?.morningCeiling===true,mode:modes.includes(v?.mode)?v.mode:modes[0],games:Math.max(0,Math.floor(Number(v?.games)||0)),level:v?.level==='high'?'high':'low',highLeft:v?.level==='high'?Math.max(0,Math.floor(Number(v?.highLeft)||0)):0,impurity:Math.min(100,Math.max(0,Number(v?.impurity)||0))};}
 ...
 function spin(value,flow,setting=1,options={},rng=Math.random,forced=''){
  ...
  if(before.games+1>=ceiling(before)){token.internalBonus=bonus((before.morningCeiling?'朝一':before.mode)+' 天井'+ceiling(before)+'G');return token;}
