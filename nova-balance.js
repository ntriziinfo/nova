/* Normal-mode calibration: see CALIBRATION-V24.md (V23 long-run calibration is historical). Targets are not guarantees. */
globalThis.NovaBalance=(()=>{
 ...
 function profile(setting){const i=Math.max(0,Math.min(5,Math.round(Number(setting)||1)-1)),scale=.21*(1+.12*NovaArt.settingBias[i]);return {setting:i+1,target:targets[i],scale,directDenom:NovaArt.defaults['direct'+(i+1)]/scale,czDenom:120/scale,strongDenom:600/scale,verifiedModel:'strong-rare-v24-frequency-verified'};}
