...
globalThis.NovaBalance=(()=>{
 ...
 function giruMean(setting,ura=false){let total=0;for(let base=1;base<=5;base++){let g=base,survival=1,mean=0;while(g<(ura?640:320)){const p=NovaArt.giruChance(g,setting,ura);mean+=survival*(1-p)*Math.max(g,NovaArt.guarantees[ura?'ura':'normal'].giru);survival*=p;g*=2;}const p=NovaArt.giruChance(g,setting,ura);total+=(mean+survival*g*(1-p)/(1-2*p))/5;}return total;}
 // Entry scales fitted with the normal-mode / ceiling / impurity / freeze simulation.
 function profile(setting){const i=Math.max(0,Math.min(5,Math.round(Number(setting)||1)-1)),scale=.21*(1+.12*NovaArt.settingBias[i]);return {setting:i+1,target:targets[i],scale,directDenom:NovaArt.defaults['direct'+(i+1)]/scale,czDenom:120/scale,strongDenom:600/scale,verifiedModel:'guarantees-v7'};}
