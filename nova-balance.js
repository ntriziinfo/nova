...
globalThis.NovaBalance=(()=>{
 ...
 function zoneMean(id,options={}){const a=NovaArt,c=a.config(options),s=a.startZone(a.enter(),id,c,()=>.5),r=a.zoneRules(s,c);
  if(r.family==='ladder')return a.ladders.reduce((sum,l)=>sum+l[0]+l.slice(1).reduce((v,n,i)=>v+(n-l[i])*r.success**(i+1),0),0)/a.ladders.length;
 ...
 function profile(setting){const i=Math.max(0,Math.min(5,Math.round(Number(setting)||1)-1)),scale=.21*(1+.12*NovaArt.settingBias[i]);return {setting:i+1,target:targets[i],scale,directDenom:NovaArt.defaults['direct'+(i+1)]/scale,czDenom:120/scale,strongDenom:600/scale,verifiedModel:'zone-v2-monte-carlo'};}
