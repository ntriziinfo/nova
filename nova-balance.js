...
globalThis.NovaBalance=(()=>{
 // Aggregate estimate with +10,000pt stop or 30,000G cutoff; 1,000 trials/setting.
 const targets=[.951028,.974137,.983669,1.013095,1.041310,1.065792];
 ...
 function profile(setting){const i=Math.max(0,Math.min(5,Math.round(Number(setting)||1)-1)),scale=.21*(1+.12*NovaArt.settingBias[i]);return {setting:i+1,target:targets[i],scale,directDenom:NovaArt.defaults['direct'+(i+1)]/scale,czDenom:120/scale,strongDenom:600/scale,verifiedModel:'comeback114-30000g-complete-stop'};}
