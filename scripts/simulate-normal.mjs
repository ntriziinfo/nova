...
export function simulate(setting,scale,games=1000000,seed=1234567){
 ...
 function bonus(kind,freeze=false){count++;const claim=n.claim(state,freeze,rng);if(state.impurity===100)release++;state=flow.phase==='art'?claim.state:n.afterBonus(claim.state,rng);let sets=claim.sets;for(let bonusPaid=0;bonusPaid<a.bonusTarget(kind);){const r=a.drawBonus(rng,setting),pay=n.pay(r);bonusPaid+=pay;paid+=pay;count++;bonusG++;bonusP+=pay;track();if(r==='NEBULA')sets++;}const beforeResume=flow;flow=a.afterBonus(flow,c,sets);state=n.afterArt(state,beforeResume,flow);if(flow.phase==='art'&&claim.zones.length){flow.queuedZones.push(...claim.zones);if(!flow.zone)flow=a.startZone(flow,flow.queuedZones.shift(),{...c,allowUra:true},rng);}}
 while(count<games){
  track();if(flow.phase==='art'&&artStart===null)artStart=paid-count*3;else if(flow.phase!=='art')artStart=null;
  if(flow.phase==='art'){flow=a.prepareBet(flow,c,rng);if(!flow.zero)count++;const s=a.step(flow,c,rng);paid+=n.pay(s.result);state=n.afterArt(state,flow,s.flow);flow=s.flow;if(s.internalBonus)bonus('BIG');continue;}
